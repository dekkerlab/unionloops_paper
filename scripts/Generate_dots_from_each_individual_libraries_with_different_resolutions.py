...
for i in dots_dict.keys():
    filename=os.path.join(output_folder,os.path.basename(input_cooler_paths.loc[input_cooler_paths.name == i,'path'].copy().reset_index(drop=True)[0])).replace('.1000.mcool', f'.{int(resolution/1000)}kb.dots.tsv')
...
df.to_csv(os.path.join(output_folder,f'dots_meta_{int(resolution/1000)}kb.tsv'), sep='\t', index=False, header=True)
