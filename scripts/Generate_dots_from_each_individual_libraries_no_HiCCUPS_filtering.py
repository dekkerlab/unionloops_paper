...
for i in range(len(input_cooler_paths)):
    ...
    dots_dict[input_cooler_paths.name[i]] = cooltools.dots(
        ...
        cluster_filtering=False, # without HiCCUPS filtering
        nproc=4,)


dots_filenames=[]
for i in dots_dict.keys():
    filename=os.path.join(output_folder,os.path.basename(input_cooler_paths.loc[input_cooler_paths.name == i,'path'].copy().reset_index(drop=True)[0])).replace('.1000.mcool', '.5kb.dots.no.HiCCUPS.filtering.tsv')
...
df.to_csv(os.path.join(output_folder,'dots_meta_5kb_no_HiCCUPS_filtering.tsv'), sep='\t', index=False, header=True)
